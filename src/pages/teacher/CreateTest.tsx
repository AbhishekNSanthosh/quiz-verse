...
import { Upload, Trash, Zap, Plus, Hash, Check, AlertCircle, Eye, File, Loader2, Edit } from "lucide-react";
...
const CreateTest = () => {
  ...
  const [questionInputMethod, setQuestionInputMethod] = useState<"manual" | "upload">("manual");
  ...
  const [optionSelectionMode, setOptionSelectionMode] = useState<"manual" | "scan">("manual");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (let i = 0; i < questions.length; i++) {
        ...
        if (question.id) {
          ...
          if (existingOptions.length > question.options.length) {
            for (let j = question.options.length; j < existingOptions.length; j++) {
              await supabase
                .from('answer_options')
                .delete()
                .eq('id', existingOptions[j].id);
      ...
      toast.success(isEditMode ? "Quiz updated successfully!" : "Quiz created successfully!", { duration: 2000 });
      navigate(`/teacher/tests`);
    } catch (error: any) {
      console.error(isEditMode ? "Error updating quiz:" : "Error creating quiz:", error);
      toast.error(error.message || (isEditMode ? "Failed to update quiz" : "Failed to create quiz"), { duration: 2000 });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const addQuestion = () => {
    ...
    if (emptyOptionIndex !== -1) {
      toast.error(`Please fill in option ${emptyOptionIndex + 1}`, { duration: 2000 });
      return;
    }
  ...
  const handleInsertQuestion = (extractedQuestion: ExtractedQuestion) => {
    ...
    const questionToInsert = {
      ...extractedQuestion,
      text: extractedQuestion.text.substring(extractedQuestion.text.indexOf('.') + 1).trim()
    };

    // Check if the question has an answer selected
    if (questionToInsert.correctOptionIndex === null && questionToInsert.options.length > 0) {
      toast.warning("No correct answer selected. First option will be set as correct by default.");
    ...
    if (editingQuestionIndex !== null) {
      ...
      updatedQuestions[editingQuestionIndex] = {
        ...
        options: questionToInsert.options.length > 0 ? 
          questionToInsert.options : 
          updatedQuestions[editingQuestionIndex].options,
        correctOptionIndex: questionToInsert.correctOptionIndex !== null ? 
  ...
  const handleInsertAllQuestions = (questions: ExtractedQuestion[]) => {
    ...
    toast.success(`${cleanedQuestions.length} questions added to quiz`);
  };
        ...
        <div className="container py-6 space-y-6">
          ...
          <Card>
            <form onSubmit={handleSubmit}>
              <CardHeader>
                <CardTitle>{isEditMode ? "Quiz Details" : "Quiz Details"}</CardTitle>
                <CardDescription>
                  {isEditMode 
              ...
              <CardHeader className="border-t">
                ...
                <CardDescription>
                  {editingQuestionIndex !== null ? 
                    "Edit question" : 
                    (isEditMode ? "Update existing or add new questions to your quiz." : "Add questions to your quiz.")}
                </CardDescription>
                <Tabs 
                  defaultValue="manual" 
                  className="mt-4"
                  value={questionInputMethod}
                  onValueChange={(value) => setQuestionInputMethod(value as "manual" | "upload")}
                >
                  <TabsList>
                    <TabsTrigger value="manual">Add Manually</TabsTrigger>
                    <TabsTrigger value="upload">
                      Upload Questions
                      <Badge variant="outline" className="ml-2 bg-primary/10">Image/PDF</Badge>
                    </TabsTrigger>
                  </TabsList>
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          <Label htmlFor="questionText">
                            {editingQuestionIndex !== null ? "Edit Question" : "Question Text"}
                          </Label>
                          {editingQuestionIndex !== null && (
                            <Button 
                              type="button" 
                      ...
                      <div className="space-y-4">
                        <div className="p-2 rounded-md bg-amber-50 border border-amber-200 mb-3">
                          <p className="text-sm text-amber-800 flex items-center">
                            ...
                            <span>Upload an image or PDF with math questions. The system will extract questions, options and you can select the correct answer.</span>
                          </p>
                        </div>
                          ...
                          <div className="ml-6 space-y-1">
                            {q.options.map((option: string, optIndex: number) => (
                              <div key={optIndex} className="flex items-center gap-2">
                                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${optIndex === q.correctOptionIndex ? 'bg-green-100 text-green-800 border border-green-300' : 'bg-muted'}`}>
                                  {optIndex === q.correctOptionIndex && <Check className="h-3 w-3" />}
                                </div>
                                <span>
                                  <MathJax>{option}</MathJax>
                                </span>
